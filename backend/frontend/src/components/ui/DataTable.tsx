...
export function DataTable<T extends object>({
  ...
  // Unique key for localStorage based on columns (or optionally table id)
  const localStorageKey = React.useMemo(() => {
    // Use all accessorKeys as a unique key, or allow a prop for tableId
    const colKeys = columns.map((col: any) => col.accessorKey || col.id).join(',');
    return `datatable_hidden_columns_${colKeys}`;
  }, [columns]);
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(() => {
    ...
    // Fallback to initialHiddenColumns prop
    const initialVisibility: VisibilityState = {};
    columns.forEach((col) => {
      const accessorKey = (col as any).accessorKey as string;
      if (accessorKey) {
        initialVisibility[accessorKey] = !initialHiddenColumns.includes(accessorKey);
      }
    });
    return initialVisibility;
  });

  // Persist columnVisibility to localStorage whenever it changes
  React.useEffect(() => {
    try {
...
const handleExport = async () => {
  ...
  const visibleColumns = table
    ...
    .filter((column) =>
      ...
      column.id !== 'sno' &&
      column.id !== 'actions' &&
      !!(column.columnDef as any).accessorKey // Only columns with accessorKey
    )
    .map((column) => ({
      header:
        typeof column.columnDef.header === 'string'
          ...
          : column.id,
      accessorKey: (column.columnDef as any).accessorKey,
    }));

  // Prepare header row
  const headers = ['S. No.', ...visibleColumns.map(col => col.header)];
  ...
  const dataRows = exportData.map((row: any, index: number) => {
    ...
    visibleColumns.forEach((col) => {
      let value: any;
      if (col.accessorKey === 'taxName') {
        value = row.tax?.taxName || '';
      } else if (col.accessorKey === 'active') {
        value = row.active === 1 ? 'Active' : row.active === 0 ? 'Inactive' : row.active;
      } else {
      ...
      rowData.push(typeof value === 'object' && value !== null ? '' : value !== undefined ? value : '');
    });
    return rowData;
  });
                  ...
                  .map((column) => (
                    <DropdownMenuItem
                      ...
                      onClick={() => {
                        ...
                        // setColumnVisibility will trigger useEffect to persist
                      }}
                    >
                      <span style={{ opacity: column.getIsVisible() ? 1 : 0.5 }}>
                        {typeof column.columnDef.header === 'string'
                          ? column.columnDef.header
                          : column.id}
                      </span>
